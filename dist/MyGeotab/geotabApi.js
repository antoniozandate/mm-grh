 ...
 */
GeotabApi = function GeotabApi(getCredentialsCallback, newOptions, customCredentialStore) {
  'use strict';
   ...
   */
  handleError = function handleError(error, errorCallback) {
    var errorString;
   ...
   */
  getCallUrl = function getCallUrl(method) {
   ...
   */
  cleanupCall = function cleanupCall(uid) {
    // Remove this function once we're done with it
    var script = document.getElementById(uid);

    if (script) {
      script.parentNode.removeChild(script); // Manually garbage-collect the script
   ...
   */
  callJSONP = function callJSONP(method, params, callbackSuccess, callbackError) {
    var uid = 'json' + (Math.random() * 100).toString().replace(/\./g, ''),
        buildParamString = function buildParamString() {
      params = params || {};
      var query = [];
   ...
   *  @return {Object} An object with operations for the call. Supported operation(s): abort()
   */
  callXHR = function callXHR(method, params, callbackSuccess, callbackError) {
   ...
   *  @return {Object} An object with operations for the call. Supported operation(s): abort()
   */
  callBase = function callBase(method, params, callbackSuccess, callbackError) {
    if (options.jsonp) {
   ...
   *  @return {Object} An object with operations for the call. Supported operation(s): abort()
   */
  authenticateWithServer = function authenticateWithServer(newServer, database, username, password, callbackSuccess, callbackError) {
    server = newServer; // Authenticate the user
   ...
   */
  populateAuthenticationCallback = function populateAuthenticationCallback(callbackSuccess) {
   ...
   *  @return {Object} An object with operations for the call. Supported operation(s): abort()
   */
  call = function call(method, params, callbackSuccess, callbackError) {
    var needsLoginAndCall = function needsLoginAndCall() {
      ...
      if (storedCredentials && options.rememberMe) {
        ...
        server = storedCredentials.server; // Try again
    ...
    return callBase(method, params, callbackSuccess, function (errorString, errorObject) {
      ...
      callAsync = function callAsync(method, params) {
    return new Promise((resolve, reject) => {
      call(method, params, success => resolve(success), error => reject(error));
    });
  },
   ...
   *  @return {Object} An object with operations for the call. Supported operation(s): abort()
   */
  multiCall = function multiCall(calls, callbackSuccess, callbackError) {
    ...
    return call('ExecuteMultiCall', {
      ...
      multiCallAsync = function multiCallAsync(calls) {
    return new Promise((resolve, reject) => {
      multiCall(calls, success => resolve(success), error => reject(error));
    });
  },
   ...
   */
  getSession = function getSession(callbackSuccess, newSession) {
    var storedCredentials = credentialsStore.get();
  ...
  var defaultCredentialsStore = {
     ...
     */
    set: function set(credentials, server) {
  ...
  credentialsStore = customCredentialStore || defaultCredentialsStore; // Public methods
