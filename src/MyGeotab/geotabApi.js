	...
	var JSONP_REQUESTS_PROPERTY_STR = 'geotabJSONP',
		...
		handleError = function (error, errorCallback) {
			...
			if (error && error.name && error.message) {
				...
				error.target ||
				(error instanceof XMLHttpRequest && error.status === 0)
			) {
				errorString =
					"Network Error: Couldn't connect to the server. Please check your network connection and try again."
			}
			if (options.debug) {
					...
					if (window[JSONP_REQUESTS_PROPERTY_STR].hasOwnProperty(uid)) {
						window[JSONP_REQUESTS_PROPERTY_STR][uid]({
							error: {
								name: 'JSONPTimeout',
								message:
									'Could not complete the JSONP request in a timely manner (' +
									options.timeout +
									's)',
			...
			return callBase(
				...
				function (errorString, errorObject) {
					...
					if (
						errors &&
						errors[0] &&
						errors[0].name === 'InvalidUserException' &&
						method !== 'Authenticate'
					) {
